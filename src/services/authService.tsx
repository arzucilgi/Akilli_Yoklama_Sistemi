...
export async function signUpWithEmail({ name, surname, email, password }: SignUpData) {
...
export const getAttendanceByCourseAndDate = async (courseId: string, date: string) => {
  const { data, error } = await supabase
    .from('attendances')
    .select('*')
    .eq('course_id', courseId)
...
export const getWeeklyAttendanceStats = async (
  ...
  const stats = {
    week: `${start.toISOString().split('T')[0]} - ${end.toISOString().split('T')[0]}`,
    Katıldı: 0,
    'Geç Kaldı': 0,
    Katılmadı: 0,
  };
