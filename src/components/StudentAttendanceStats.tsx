...
const StudentAttendanceStats: React.FC = () => {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'email', headerName: 'Email', flex: 1.5 ,headerClassName:'headerStyles' },
    { field: 'number', headerName: 'Öğrenci No', flex: 1,headerClassName:'headerStyles' },
    { field: 'Katıldı', headerName: 'Katıldı', type: 'number', flex: 0.8,headerClassName:'headerStyles' },
    { field: 'Geç Kaldı', headerName: 'Geç Kaldı', type: 'number', flex: 0.8,headerClassName:'headerStyles' },
    { field: 'Katılmadı', headerName: 'Katılmadı', type: 'number', flex: 0.8,headerClassName:'headerStyles' },
  ];
  ...
  return (
    ...
    >
      ...
      {!loading && !error && stats.length > 0 && (
        <>
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={stats} layout="horizontal" margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
      ...
      {!loading && !error && stats.length === 0 && (
        <Typography>Seçilen derse ait öğrenci katılım verisi bulunamadı.</Typography>
      )}
    </Card>
  );
};
